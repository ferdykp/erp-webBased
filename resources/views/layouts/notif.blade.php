    ...
    @foreach ($alerts as $type => $classes)
        @if (session($type))
            ...
            setTimeout(() => show = false, 3500)" x-show="show"
                ...
                x-transition:leave-end="opacity-0 translate-y-2 scale-95" {{-- class="px-10 py-5 text-md font-medium border shadow-xl rounded-xl {{ $classes }}">
                {{ session($type) }} --}}
                ...
                <i class="mr-2 fa-solid fa-circle-check"></i> {{ session($type) }}
