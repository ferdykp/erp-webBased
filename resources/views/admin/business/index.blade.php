...
@section('content')
    <div class="w-full pb-10 space-y-8" x-data="{ openModal: false, selectedCustomer: {} }">
        ...
        <div class="flex flex-col justify-between gap-4 px-2 md:flex-row md:items-center">
            <div>
                <h2 class="text-3xl font-black tracking-tight text-slate-800">Customer Monitoring</h2>
            ...
            <form action="{{ route('admin.business.index') }}" method="GET" class="relative group">
                ...
                <i
                    class="absolute -translate-y-1/2 fa-solid fa-magnifying-glass left-5 top-1/2 text-slate-400 group-focus-within:text-indigo-500"></i>
            </form>
        </div>
                        ...
                        <tr class="transition-colors hover:bg-slate-50/80 group">
                            <td class="px-8 py-6">
                                <div class="flex flex-col">
                                    <span
                                        class="text-base font-bold text-slate-800">{{ $customer->company_name ?? 'Unnamed Company' }}</span>
                            ...
                            <td class="px-6 py-6 text-center">
                                <span
                                    class="{{ $customer->status === 'active' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600' }} px-3 py-1 rounded-lg text-[10px] font-black uppercase">
        ...
        <div x-show="openModal" x-transition:enter="transition ease-out duration-300"
            ...
            <div @click.away="openModal = false"
                class="bg-slate-50 w-full max-w-5xl max-h-[90vh] overflow-y-auto rounded-[3rem] shadow-2xl border border-white">

                <div class="sticky top-0 z-10 flex items-center justify-between p-8 bg-white border-b border-slate-100">
                    <div class="flex items-center gap-4">
                        <div
                            class="flex items-center justify-center w-12 h-12 text-white bg-indigo-500 shadow-lg rounded-2xl shadow-indigo-200">
                            <i class="text-xl fa-solid fa-building"></i>
                        </div>
                        <div>
                ...
                <div class="p-8 space-y-6">
                    <section class="grid grid-cols-1 gap-6 md:grid-cols-3">
                        <div
                            class="md:col-span-2 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
                            <h4 class="text-[10px] font-black text-indigo-500 uppercase tracking-widest">Company Profile
                                Details</h4>
                            <div class="grid grid-cols-2 gap-y-6 gap-x-4">
                                ...
                                <div>
                                    <label class="text-[10px] text-slate-400 font-bold uppercase block mb-1">Profile
                                        Verification</label>
                                    <span
                                        :class="selectedCustomer.profile_completed ? 'text-emerald-500' : 'text-amber-500'"
                        ...
                        <div class="bg-indigo-600 p-8 rounded-[2.5rem] shadow-lg shadow-indigo-100 text-white space-y-4">
                            ...
                            <i class="float-right mt-4 text-4xl fa-solid fa-map-location-dot opacity-20"></i>
                        </div>
                    </section>

                    <section class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                        <h4 class="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-6">Product History
                            Monitoring</h4>
