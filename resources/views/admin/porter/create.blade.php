...
@section('title', 'Onboard New Porter')
    ...
    <div class="max-w-xl pb-20 mx-auto space-y-6">
        ...
        <div>
            ...
            <h2 class="text-2xl font-black tracking-tight text-slate-800">
                Onboard <span class="text-indigo-600">New Porter</span>
            </h2>
            <p class="text-xs text-slate-400 mt-0.5">Initialize profile records for ground handling operators.</p>
        </div>

        {{-- FORM CARD --}}
        <div class="p-6 overflow-hidden bg-white border shadow-sm border-slate-200/70 rounded-2xl sm:p-8">
            <form action="{{ route('admin.porter.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="p-4 bg-slate-50/60 border border-slate-100 rounded-xl flex items-start gap-3.5 group">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] text-slate-400 leading-normal mt-0.5">
                            If enabled, this porter will be flagged as **Active** and made available for assignment logs
                            across active manifests instantly.
                        </p>
                    </div>
                </div>
