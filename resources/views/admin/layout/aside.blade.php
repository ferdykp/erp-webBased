    ...
    <nav class="flex-1 overflow-y-auto px-3 py-5 [scrollbar-width:thin]">
        <div class="mb-6 space-y-1">
            ...
            <a href="{{ route('admin.dashboard') }}"
                ...
                <i
                    class="fa-solid fa-table-cells-large w-5 shrink-0 text-center text-[13px] {{ request()->routeIs('admin.dashboard') ? 'text-white' : 'text-slate-500' }}"></i>
                <span>Dashboard</span>
            </a>
            ...
            <div x-data="{ open: {{ request()->routeIs('admin.bookings*') ? 'true' : 'false' }} }">
                <button @click="open = !open"
                    ...
                    <i
                        class="fa-solid fa-clipboard-list w-5 shrink-0 text-center text-[13px] {{ request()->routeIs('admin.bookings*') ? 'text-blue-400' : 'text-slate-500' }}"></i>
            ...
            <a href="{{ route('admin.testing.index') }}"
                ...
                <i
                    class="fa-solid fa-flask-vial w-5 shrink-0 text-center text-[13px] {{ request()->routeIs('admin.testing.*') ? 'text-white' : 'text-slate-500' }}"></i>
            ...
            @if (in_array($role, ['superadmin', 'production']))
                <div x-data="{ open: {{ request()->routeIs('admin.production*') ? 'true' : 'false' }} }">
                    <button @click="open = !open"
                        ...
                        <i
                            class="fa-solid fa-industry w-5 shrink-0 text-center text-[13px] {{ request()->routeIs('admin.production*') ? 'text-blue-400' : 'text-slate-500' }}"></i>
                ...
                <a href="{{ route('admin.dosimeter.index') }}"
                    ...
                    <i
                        class="fa-solid fa-wave-square w-5 shrink-0 text-center text-[13px] {{ request()->routeIs('admin.dosimeter.*') ? 'text-white' : 'text-slate-500' }}"></i>
                    <span>Dosimeter</span>
                </a>
            @endif
        </div>
        ...
        <div class="mb-6 space-y-1">
            ...
            <a href="{{ route('admin.customerList.index') }}"
                ...
                <i
                    class="fa-solid fa-building-user w-5 shrink-0 text-center text-[13px] {{ request()->routeIs('admin.customerList.*') ? 'text-white' : 'text-slate-500' }}"></i>
            ...
            @if (in_array($role, ['superadmin', 'cargo_admin']))
                <a href="{{ route('admin.pallets.index') }}"
                    ...
                    <i
                        class="fa-solid fa-boxes-stacked w-5 shrink-0 text-center text-[13px] {{ request()->routeIs('admin.pallets.*') ? 'text-white' : 'text-slate-500' }}"></i><span>Pallet
                        Control</span>
                </a>
                <a href="{{ route('admin.porter.index') }}"
                    class="{{ $navBase }} {{ request()->routeIs('admin.porter.*') ? $navActive : $navIdle }}">
                    <i
                        class="fa-solid fa-people-carry-box w-5 shrink-0 text-center text-[13px] {{ request()->routeIs('admin.porter.*') ? 'text-white' : 'text-slate-500' }}"></i><span>Porter</span>
                </a>
                <a href="{{ route('admin.warehouse-pics.index') }}"
                    class="{{ $navBase }} {{ request()->routeIs('admin.warehouse-pics.*') ? $navActive : $navIdle }}">
                    <i
                        class="fa-solid fa-user-shield w-5 shrink-0 text-center text-[13px] {{ request()->routeIs('admin.warehouse-pics.*') ? 'text-white' : 'text-slate-500' }}"></i><span>Warehouse
                        PIC</span>
                </a>
            @endif
        </div>
            ...
            <div class="mb-6 space-y-1">
                ...
                <div x-data="{ open: {{ request()->routeIs('admin.report.*') ? 'true' : 'false' }} }">
                    <button @click="open = !open"
                        ...
                        <i
                            class="fa-solid fa-file w-5 shrink-0 text-center text-[13px] {{ request()->routeIs('admin.report.*') ? 'text-blue-400' : 'text-slate-500' }}"></i>
                        <span>Reports</span>
                        <i class="fa-solid fa-chevron-down ml-auto text-[8px] text-slate-600 transition-transform duration-200"
                            :class="open ? 'rotate-180' : ''"></i>
                    </button>
        ...
        @if (in_array($role, ['superadmin', 'manager']))
            <div class="mb-0 space-y-1">
                ...
                <a href="{{ route('admin.business.index') }}"
                    ...
                    <i
                        class="fa-solid fa-chart-line w-5 shrink-0 text-center text-[13px] {{ request()->routeIs('admin.business.*') ? 'text-white' : 'text-slate-500' }}"></i><span>Business
                        Monitoring</span>
                </a>
                <a href="{{ route('admin.production-lines.index') }}"
                    class="{{ $navBase }} {{ request()->routeIs('admin.production-lines.*') ? $navActive : $navIdle }}">
                    <i
                        class="fa-solid fa-microchip w-5 shrink-0 text-center text-[13px] {{ request()->routeIs('admin.production-lines.*') ? 'text-white' : 'text-slate-500' }}"></i><span>Production
                        Units</span>
                </a>
                <a href="{{ route('admin.profile.profileList') }}"
                    ...
                    <i
                        class="fa-solid fa-user-gear w-5 shrink-0 text-center text-[13px] {{ request()->routeIs('admin.profile.profileList') || request()->routeIs('admin.profile.edit') ? 'text-white' : 'text-slate-500' }}"></i><span>Staff
                        Accounts</span>
                </a>
            </div>
        @endif
    ...
    <div class="p-3 border-t shrink-0 border-slate-800/80">
        <div class="flex items-center gap-3 px-3 py-3 rounded-xl bg-slate-900">
            ...
            <div class="flex-1 min-w-0">
                <p class="text-xs font-semibold truncate text-slate-200">{{ $adminUser?->name }}</p>
